...
export async function POST(req: Request) {
  try {
    const keyId = process.env.RAZORPAY_KEY_ID || 'rzp_live_SXiE7TgmHMi6sw';
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ success: false, error: 'Failed to create order with Razorpay API.' }, { status: res.status });
    }
